...
class FiLMedNet(nn.Module):
  def __init__(self, vocab, feature_dim=(1024, 14, 14),
    ...
    self.coords = coord_map((module_H, module_W), device=self.device)
    self.default_weight = Variable(torch.ones(1, 1, self.module_dim)).type(torch.FloatTensor).to(self.device)
    self.default_bias = Variable(torch.zeros(1, 1, self.module_dim)).type(torch.FloatTensor).to(self.device)
  ...
  def forward(self, x, film, save_activations=False):
    ...
    module_inputs = Variable(torch.zeros(feats.size()).unsqueeze(1).expand(
      N, self.num_modules, self.module_dim, H, W)).type(torch.FloatTensor).to(self.device)
...
def coord_map(shape, start=-1, end=1, device='cuda'):
  """
  Gives, a 2d shape tuple, returns two mxn coordinate maps,
  Ranging min-max in the x and y directions, respectively.
  """
  m, n = shape
  x_coord_row = torch.linspace(start, end, steps=n).type(torch.FloatTensor).to(device)
  y_coord_row = torch.linspace(start, end, steps=m).type(torch.FloatTensor).to(device)
